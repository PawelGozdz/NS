...
export class CreateSkillHandler implements IInferredCommandHandler<CreateSkillCommand> {
  ...
  async execute(command: CreateSkillCommand): Promise<CreateSkillResponseDto> {
    ...
    const category = await this.queryBus.execute(new GetManyCategoriesQuery({ _filter: { id: command.categoryId }, actor }));
    ...
    await this.outboxRepository.store(this.createOutbox(new SkillCreatedEvent({ id: skill.id, ...instance, actor })));
