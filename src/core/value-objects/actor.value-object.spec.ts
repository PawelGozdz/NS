import { ActorType, MissingValueError } from '@libs/common';
...
describe('Actor', () => {
  describe('create', () => {
    it('should throw MissingValueError if type is missing', () => {
      expect(() => Actor.create(undefined as unknown as ActorType, 'Source')).toThrow(MissingValueError);
    });

    it('should throw ActorError if type is invalid', () => {
      expect(() => Actor.create('INVALID_TYPE' as ActorType, 'Source')).toThrow(ActorError);
    });
