	...
	describe('getCookies', () => {
		describe('success', () => {
			it('should get cookies', () => {
				...
				// Act
				const result = service.getCookies({ access_token, refresh_token });

				// Assert
				expect(result).toEqual([accessCookie.cookie, refreshCookie.cookie]);
				expect(service.getCookieWithJwtAccessToken).toHaveBeenCalledWith(access_token);
				expect(service.getCookieWithJwtRefreshToken).toHaveBeenCalledWith(refresh_token);
